...
const CryptoDetails = () => {
  ...
  const { id } = router.query; // Assuming the URL is like /crypto/[id]
  ...
  useEffect(() => {
    ...
    if (router.isReady && id) {
      getCoinData(id.toString()).finally(() => setIsLoading(false));
    }
  ...
  return (
    <div
      className="fixed inset-0 bg-gray-200 bg-opacity-30 backdrop-blur-sm flex items-center justify-center"
      onClick={() => router.back()}
    >
      <div
      ...
      >
         <button onClick={() => router.back()} className="absolute top-3 right-3 text-white">
